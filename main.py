...
def parse_arguments():
    ...
    parser.add_argument("--mode", type=str, default="decompose", help="실행 모드 (decompose / prune)")
...
def inject_decomposable_empty_layers(model, decomposable_layers, max_ranks, args):
    for idx in tqdm(range(len(decomposable_layers)), desc="Injecting decomposable layers"):
        parent_layer, last_token = decomposable_layers[idx]
        new_layer = ModuleInjection.make_decomposable_empty(
            getattr(parent_layer, last_token), max_ranks[idx], args.algo
        )
        setattr(parent_layer, last_token, new_layer)
    for _, param in model.named_parameters():
...
def main():
    ...
    if not os.path.exists(decomposed_weights_name):
        print(f"[INFO] Decomposed model file {decomposed_weights_name} not found. Running forward test and saving model.")
        ...
        #base_model = torch.load(decomposed_weights_name + ".pt")
        model = AutoModelForCausalLM.from_pretrained(
            args.model,
            torch_dtype=torch.float32,
            device_map="cuda",
        ...
        decomposable_layers_base, max_ranks = get_decomposable_layers(model, layer_names)
        inject_decomposable_empty_layers(base_model, decomposable_layers_base, max_ranks, args)
        decomposed_sd = torch.load(os.path.join(args.save_folder, "pytorch_model.bin"), map_location="cpu")
        missing, unexpected = model.load_state_dict(decomposed_sd, strict=False)
